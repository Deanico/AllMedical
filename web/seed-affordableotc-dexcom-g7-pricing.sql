-- Seed: AffordableOTC Dexcom G7 pricing
-- Safe behavior:
...
WITH input_base AS (
  SELECT *
  FROM (VALUES
    ('Dexcom G7 Sensors (10-day)'::text, 119.97::numeric(10,2)),
    ('Dexcom G7 Sensors (15-day)'::text, 149.96::numeric(10,2))
...
matched AS (
  ...
  JOIN vendors vnd ON vnd.name = 'AffordableOTC'
)
...
SELECT
  ...
  m.base_price,
  true,
  'https://affordableotc.com/search?type=product&q=G7+sensor',
  'manual',
  true,
  NOW()
FROM matched m
...
WITH input_tiers AS (
  SELECT *
  FROM (VALUES
    ('Dexcom G7 Sensors (10-day)'::text, 1::int, NULL::int, 119.97::numeric(10,2)),
    ('Dexcom G7 Sensors (10-day)'::text, 3::int, 3::int,    116.66::numeric(10,2)),
    ('Dexcom G7 Sensors (15-day)'::text, 1::int, NULL::int, 149.96::numeric(10,2))
