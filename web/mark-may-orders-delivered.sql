-- Mark May 2026 orders as delivered for all clients except the listed exclusions.
...
WHERE po.ship_date >= DATE '2026-05-01'
  AND po.ship_date < DATE '2026-06-01'
...
WHERE l.id = po.lead_id
  AND po.ship_date >= DATE '2026-05-01'
  AND po.ship_date < DATE '2026-06-01'
  AND po.status <> 'cancelled'
  AND NOT (LOWER(l.name) ~ '(^|[[:space:]])(genevieve|fulmer|tammy|carter)([[:space:]]|$)');
